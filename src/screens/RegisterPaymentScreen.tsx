...
export function RegisterPaymentScreen() {
  ...
  const handleSubmit = async () => {
    ...
    const amount = Number(valorPagado.replace(/\./g, '').replace(/,/g, ''));

    if (!Number.isFinite(amount) || amount <= 0) {
      Alert.alert('Valor inválido', 'Ingresa un valor de pago válido.');
      return;
    }

    if (selectedCredit && amount > selectedCredit.saldoPendiente) {
      Alert.alert(
        'Pago mayor al saldo',
        `El saldo pendiente es ${formatMoney(selectedCredit.saldoPendiente)}. Ingresa un valor menor o igual.`
      );
      return;
    }
        ...
        {selectedClient && useClientFilter ? (
          <Card style={styles.clientCard}>
            ...
            <Text style={styles.filterText}>
              Mostrando solo créditos de este cliente.
            </Text>
          ...
          <Card style={styles.clientCard}>
            ...
            <Text style={styles.filterText}>
              Ahora estás viendo créditos de todos los clientes.
            </Text>
                ...
                <View style={styles.creditHeader}>
                  <View style={styles.creditInfo}>
                    ...
                    <Text style={styles.creditMeta}>
                      Prestado: {formatMoney(credit.valorPrestado)}
                    </Text>

                    <Text style={styles.creditMeta}>
                      Total: {formatMoney(credit.valorTotal)}
                    </Text>

                    <Text style={styles.creditBalance}>
                      Saldo: {formatMoney(credit.saldoPendiente)}
                    </Text>
                  </View>

                  <StatusBadge
                    type={credit.estado === 'vencido' ? 'danger' : 'warning'}
                    label={credit.estado}
                  />
                </View>

                <Text style={styles.creditMeta}>
                  Cuota: {formatMoney(credit.valorCuota)} · {credit.frecuencia}
                </Text>

                {selected ? (
                  <Text style={styles.selectedText}>Crédito seleccionado</Text>
                ) : null}
              </Pressable>
            );
          })
        ...
        <Card style={styles.formCard}>
          ...
          {selectedCredit ? (
            <View style={styles.balanceBox}>
              ...
              <Text style={styles.balanceValue}>{formatMoney(selectedCredit.saldoPendiente)}</Text>
            </View>
          ) : null}
